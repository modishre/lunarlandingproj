...
from gym.envs.box2d.lunar_lander import LunarLander, heuristic
# import gym
# from gym.envs.box2d.lunar_lander import LunarLander

class CustomLunarLander(LunarLander):
    def __init__(self):
        super(CustomLunarLander, self).__init__(render_mode = 'human' )
        # self.step_count = 0  # Initialize the step counter
        self.start_time = None

    def reset(self):
        self.start_time = time.time()
        # self.step_count = 0  # Reset the step counter at the start of each episode
        return super(CustomLunarLander, self).reset()

    def step(self, action):
        # self.step_count += 1  # Increment the step counter
        result = super(CustomLunarLander, self).step(action)
        # time_penalty = -0.05 * self.step_count 

        elapsed_time = time.time() - self.start_time
        time_penalty = -0.1 * elapsed_time
